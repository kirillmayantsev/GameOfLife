...
GameOfLife = R6Class("GameOfLife",
  public = list(
    ## Initialize function
    initialize = function(n_row, n_col, square_ratio, cyclic_border) {
      ...
      private$cyclic_border = cyclic_border;
    },
    ...
    set_init_state = function(pattern = "random") {
      ...
      if (pattern == "ten_cell_row") {
        if (private$n_row >= 20 && private$n_col >= 20) {
          half_n_row = private$n_row %/% 2;
          half_n_col = private$n_col %/% 2;
          private$board_mat[half_n_row, (half_n_col - 4):(half_n_col + 5)] = 1;
        }
      }
      if (pattern == "exploder") {
        if (private$n_row >= 20 && private$n_col >= 20) {
          half_n_row = private$n_row %/% 2;
          half_n_col = private$n_col %/% 2;
          private$board_mat[(half_n_row - 2):(half_n_row + 2), c(half_n_col - 2, half_n_col + 2)] = 1;
          private$board_mat[c(half_n_row - 2, half_n_row + 2), half_n_col] = 1;
        }
      }
    ...
    plot_game_board = function() {
      ...
      out_board_plot = private$board_plot +
        geom_rect(
          data = board_df,
          mapping = aes(xmin = y - 1, xmax = y, ymin = x - 1, ymax = x, fill = as.factor(status)),
          color = "black"
        )
  ...
  private = list(
    ...
    obtain_neighbors_amount = function(status_mat) {
      ...
      if (private$cyclic_border) {
        ## take into account borders relationship in case of cyclic borders
        ...
        extended_neighbors_mat[max_row_ind, col_index_vec] =
          extended_neighbors_mat[max_row_ind, col_index_vec] +
          extended_neighbors_mat[1, col_index_vec];
        extended_neighbors_mat[min_row_ind, col_index_vec] =
          extended_neighbors_mat[min_row_ind, col_index_vec] +
          extended_neighbors_mat[n_ext_row, col_index_vec];
        extended_neighbors_mat[row_index_vec, max_col_ind] =
          extended_neighbors_mat[row_index_vec, max_col_ind] +
          extended_neighbors_mat[row_index_vec, 1];
        extended_neighbors_mat[row_index_vec, min_col_ind] =
          extended_neighbors_mat[row_index_vec, min_col_ind] +
          extended_neighbors_mat[row_index_vec, n_ext_col];
...
ui = fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxInput("square_ratio", "Square game board"),
      checkboxInput("cyclic_border", "Cyclic game borders"),
      selectizeInput(
    ...
    mainPanel(
      plotOutput("board_plot", width = "100%", height = "600px")
    )
  )
)
...
server = function(input, output) {
  ...
  observeEvent(input$create, {
    new_game <<-
      GameOfLife$new(
        n_row = input$nrow,
        n_col = input$ncol,
        square_ratio = input$square_ratio,
        cyclic_border = input$cyclic_border
      );
    game_reactive$board_ggplot_obj = new_game$plot_game_board();
    new_game$print();
  });
  ...
  observeEvent(input$start, {
    if (!is.null(new_game) && is.null(game_reactive$timer)) {
